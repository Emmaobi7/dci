...
import { FaGoogle, FaEye, FaEyeSlash, FaUser, FaEnvelope } from 'react-icons/fa';

const Signup = ({ onSwitchToLogin, onShowPrivacy, onShowTerms }) => {
  ...
  const validateForm = () => {
    if (!formData.firstName.trim()) {
      setLocalError('FIRST NAME REQUIRED');
      return false;
    }
    if (!formData.lastName.trim()) {
      setLocalError('LAST NAME REQUIRED');
      return false;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setLocalError(error.message.toUpperCase());
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex items-start space-x-3">
              <input
                ...
                className="mt-1 h-4 w-4 bg-gray-900 border-gray-600 rounded text-teal-400 focus:ring-teal-400 focus:ring-offset-gray-900"
              />
              <label htmlFor="acceptTerms" className="text-xs text-gray-400 font-mono tracking-wider">
                I AGREE TO THE{' '}
            ...
            >
              {isLoading ? (
                ...
                <>
                  <span className="group-hover:scale-110 transition-transform">CREATE ACCOUNT</span>
